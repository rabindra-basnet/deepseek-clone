...
const Sidebar = ({ expand, setExpand }) => {
    ...
    return (
        ...
        >
            <div>
                <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center"}`} >
                    <Image className={expand ? "w-36" : "w-10"} src={expand ? assets.logo_text : assets.logo_icon} alt="" />
                    <div onClick={() => (expand ? setExpand(false) : setExpand(true))} className="group relative flex items-center justify-center hover:bg-gray-500/20 transition-all duration-300  w-9 aspect-square rounded-lg cursor-pointer">
                ...
                <button className={`mt-8 flex items-center justify-center cursor-pointer ${expand ? "bg-primary hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max" : "group relative h-9 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
                    ...
                    <div className="absolute w-max -top-12 -right-12 opacity-0 group-hover:opacity-100 transistion bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none">  New Chat <div className="w-3 h-3 absolute bg-black rotate-45 left-4 -bottom-1.5"></div>
                    </div>
            ...
            <div>
                <div className={`flex items-center cursor-pointer group related ${expand ? "gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
